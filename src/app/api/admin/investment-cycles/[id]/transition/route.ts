...
export async function POST(
  ...
  try {
    ...
    const body = (await request.json()) as { status: InvestmentCycleStatus; reviewNote?: string };
    if (!body.status) {
      return NextResponse.json({ error: "status is required" }, { status: 400 });
    }
    const cycle = await investmentCycleService.adminReview(id, body.status, body.reviewNote);
