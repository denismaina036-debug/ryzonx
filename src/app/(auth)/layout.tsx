...
export default async function AuthRouteLayout({
  ...
  const [legalLinks, { contact }] = await Promise.all([
    getLegalLinksSafe(),
  ...
  return (
    <LegalLinksProvider links={legalLinks}>
      <PublicLayout contact={contact}>
        <div className="ryvonx-auth relative flex min-h-[calc(100vh-8rem)] items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_20%_10%,rgba(37,99,235,.1),transparent_28rem),radial-gradient(circle_at_85%_85%,rgba(99,102,241,.08),transparent_30rem)] px-6 py-10">
          ...
          <div className="ryvonx-auth-content w-full max-w-md">{children}</div>
        </div>
      </PublicLayout>
    </LegalLinksProvider>
  );
