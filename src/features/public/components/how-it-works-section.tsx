...
export async function HowItWorksSection({ className }: { className?: string } = {}) {
  ...
  return (
    <SectionContainer className={className ?? "bg-surface-1"} landingMobile>
      <SectionHeader
        badge={copyTradingText(content.copy.howItWorks.badge)}
        title={copyTradingText(content.copy.howItWorks.title)}
        description={copyTradingText(content.copy.howItWorks.description)}
        align="center"
      />
        ...
        {content.howItWorksSteps.map((step) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="mt-2 text-lg font-semibold text-navy-950">{copyTradingText(step.title)}</h3>
              <p className="mt-2 text-sm leading-relaxed text-navy-500">{copyTradingText(step.description)}</p>
            </div>
          );
        })}
      </div>
