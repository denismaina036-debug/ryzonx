...
import { FINANCE_SECTION_NAV } from "@/features/admin/constants/nav";

interface AdminFinanceShellProps {
  title: string;
  description?: string;
  actions?: ReactNode;
  statusNav?: ReactNode;
...
export function AdminFinanceShell({
  ...
  statusNav,
  children,
}: AdminFinanceShellProps) {
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-border bg-gradient-to-br from-white to-emerald-50/40 p-5 shadow-sm">
      ...
      <AdminSubNav
        items={FINANCE_SECTION_NAV.map((item) => ({
          label: item.label,
          href: item.href,
