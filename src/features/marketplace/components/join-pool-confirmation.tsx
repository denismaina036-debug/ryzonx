...
export function JoinPoolConfirmation({
  ...
  return (
    <>
      <div className="mx-auto w-full min-w-0 max-w-2xl space-y-5 pb-28 sm:space-y-8 sm:pb-0 lg:pb-0">
        <Link
        ...
        {!isAuthenticated ? (
          ...
          <>
            ...
            <Button
              size="lg"
              className="hidden h-12 w-full text-base sm:flex sm:h-11 sm:text-sm"
      ...
      {isAuthenticated && (
        <div className="fixed inset-x-0 bottom-[calc(3.25rem+env(safe-area-inset-bottom))] z-[45] border-t border-[var(--id-border)] bg-[var(--id-bg)]/95 p-4 shadow-[0_-8px_24px_rgba(15,23,42,0.08)] backdrop-blur-md sm:hidden">
          {error && (
            <p className="mb-3 rounded-lg bg-red-500/10 px-3 py-2 text-sm text-[var(--id-danger)]">{error}</p>
          )}
          <Button
            size="lg"
