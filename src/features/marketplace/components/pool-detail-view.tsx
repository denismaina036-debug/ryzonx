...
export function PoolDetailView({ pool }: PoolDetailViewProps) {
  ...
  const displayName = copyTraderName(pool);
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <header className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-sm text-[var(--id-text-muted)]">Strategy · {copyTradingText(pool.displayPoolName || pool.name)}</p>
      ...
      {(isHealthy || pool.poolVerified) && (
        <div className="flex flex-wrap gap-2">
          {isHealthy && (
            <TrustBadge icon={ShieldCheck} label="Strategy is healthy" variant="healthy" />
          )}
          {pool.poolVerified && (
        ...
        {pool.activeCycle ? (
          ...
          <div className="mt-3 space-y-4">
            <p className="text-sm text-[var(--id-text-muted)]">
              No copy cycle is currently accepting new participants.
            </p>
      ...
      <section className="rounded-xl border border-[var(--id-border)] bg-[var(--id-surface)] p-5 sm:p-6">
        ...
        <p className="mt-1 text-sm text-[var(--id-text-muted)]">
          Total profit realized in this strategy
        </p>
      </section>
      ...
      {pool.roiMultipliers.length > 0 && (
        ...
        >
          <h2 className="text-sm font-semibold text-[var(--id-text)]">
            Copy Levels & Copy Ratios
          </h2>
          <p className="mt-1 text-xs text-[var(--id-text-muted)]">
            Copy ratios set by the Verified Trader for each platform copy allocation level.
          </p>
      ...
      {pool.canParticipate && (
        <section className="rounded-xl border border-[var(--id-border)] bg-[var(--id-surface)] p-5 sm:p-6">
          <h2 className="text-sm font-semibold text-[var(--id-text)]">Preview Your Copy Allocation</h2>
          <p className="mt-1 text-xs text-[var(--id-text-muted)]">
