...
export function MarketplaceHero({ stats }: MarketplaceHeroProps) {
  return (
    <>
      <MobileMarketplaceHero stats={stats} />
      <DesktopMarketplaceHero stats={stats} />
    </>
  );
}

/** Compact mobile header — pools remain the visual focus. */
function MobileMarketplaceHero({ stats }: MarketplaceHeroProps) {
  return (
    <section className="md:hidden">
      <h1 className="text-[1.35rem] font-semibold leading-snug tracking-tight text-[var(--id-text)]">
        Choose the Right Investment Pool.
      </h1>
      <p className="mt-1 text-xs text-[var(--id-text-muted)]">
        Compare pools. Invest confidently.
      </p>
      <div className="mt-2.5 flex items-center gap-3 text-[11px] text-[var(--id-text-muted)]">
        <span className="inline-flex items-center gap-1.5">
          <Archive className="h-3.5 w-3.5 text-[var(--id-accent-text)]" aria-hidden />
          <span className="font-semibold tabular-nums text-[var(--id-text)]">
            {stats.livePools}
          </span>
          <span>Live Pools</span>
        </span>
        <span className="h-3 w-px shrink-0 bg-[var(--id-border)]" aria-hidden />
        <span className="inline-flex items-center gap-1.5">
...
function DesktopMarketplaceHero({ stats }: MarketplaceHeroProps) {
  return (
    <section className="relative hidden overflow-hidden rounded-[var(--id-radius)] border border-[var(--id-border)] bg-[var(--id-surface)] shadow-[var(--id-shadow-lg)] md:block">
      <div
