...
/** One server-rendered support link for public, auth and dashboard routes. */
export async function WhatsAppSupport() {
  const content = await landingPageService.getRawContent();
  const href = getWhatsAppSupportUrl(content.contact.whatsapp);
  if (!href) return null;
  ...
  return (
    <a
      ...
      aria-label="Chat with RyvonX support on WhatsApp (opens in a new tab)"
      className={styles.button}
    >
      {/* Official WhatsApp Digital Glyph, white RGB (Meta Brand Resource Center). */}
      <svg viewBox="0 0 720 720" width="32" height="32" fill="#fff" aria-hidden="true" focusable="false">
      ...
      <span className={styles.label} aria-hidden="true">WhatsApp support</span>
    </a>
  );
}
