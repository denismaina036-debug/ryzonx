...
BEGIN
  ...
  IF v_cycle.status = 'funding' THEN
    RETURN v_cycle.id;
  END IF;
  IF v_cycle.status NOT IN ('draft', 'submitted', 'approved') THEN
...
BEGIN
  ...
  WHERE candidate.fund_id = p_fund_id
    AND candidate.status IN ('draft', 'submitted', 'approved')
    AND (
...
-- Final authoritative stop guard. continue_copying_atomic records its movement
-- as copy_continue inside the same database transaction; rejecting that insert
-- rolls the complete continuation back, including allocation and balance writes.
...
AS $$
BEGIN
  IF NEW.payment_method = 'copy_continue'
    AND EXISTS (
      SELECT 1
      FROM copy_stop_requests request
      WHERE request.investor_id = NEW.user_id
        AND request.investment_cycle_id = (NEW.metadata->>'source_cycle_id')::UUID
        AND request.status = 'requested'
        AND (
          NEW.metadata->>'copy_session_id' IS NULL
          OR request.copy_session_id::TEXT = NEW.metadata->>'copy_session_id'
        )
    ) THEN
...
REVOKE ALL ON FUNCTION prevent_stopped_copy_continuation() FROM PUBLIC;

DROP TRIGGER IF EXISTS prevent_stopped_copy_continuation_before_insert ON transactions;
CREATE TRIGGER prevent_stopped_copy_continuation_before_insert
  BEFORE INSERT ON transactions
  FOR EACH ROW
  WHEN (NEW.payment_method = 'copy_continue')
