/**
 * Reconcile legacy funding wallet balances with ledger after profit transfers.
 * Usage: node scripts/reconcile-funding-wallet-balances.mjs [--apply]
 */
import { createClient } from "@supabase/supabase-js";
import { readFileSync, existsSync } from "node:fs";
import { resolve, dirname } from "node:path";
import { fileURLToPath } from "node:url";

const __dirname = dirname(fileURLToPath(import.meta.url));
const root = resolve(__dirname, "..");
const DEFAULT_FUND = "00000000-0000-4000-a000-000000000001";
const apply = process.argv.includes("--apply");

function loadEnv() {
  const path = resolve(root, ".env.local");
  if (!existsSync(path)) {
    console.error("Missing .env.local");
    process.exit(1);
  }
  let raw = readFileSync(path, "utf8");
  if (raw.charCodeAt(0) === 0xfeff) raw = raw.slice(1);
  const env = {};
  for (const line of raw.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith("#")) continue;
    const eq = trimmed.indexOf("=");
    if (eq <= 0) continue;
    const key = trimmed.slice(0, eq).trim();
    let val = trimmed.slice(eq + 1).trim();
    if (
      (val.startsWith('"') && val.endsWith('"')) ||
      (val.startsWith("'") && val.endsWith("'"))
    ) {
      val = val.slice(1, -1);
    }
    env[key] = val;
  }
  return env;
}

function n(v) {
  return typeof v === "number" ? v : Number(v ?? 0);
}

function roundMoney(value) {
  return Math.round(value * 100) / 100;
}

process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";
const env = loadEnv();
const db = createClient(env.NEXT_PUBLIC_SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY, {
  auth: { persistSession: false },
});

async function ledgerBalance(accountId, accountType) {
  const { data: entries } = await db
    .from("ledger_entries")
    .select("entry_side, amount")
    .eq("account_id", accountId);
  const debits = (entries ?? [])
    .filter((e) => e.entry_side === "debit")
    .reduce((s, e) => s + n(e.amount), 0);
  const credits = (entries ?? [])
    .filter((e) => e.entry_side === "credit")
    .reduce((s, e) => s + n(e.amount), 0);
  return accountType === "liability" ? credits - debits : debits - credits;
}

const { data: transfers, error } = await db
  .from("transactions")
  .select("user_id")
  .eq("payment_method", "profit_transfer")
  .eq("status", "completed");

if (error) {
  console.error(error.message);
  process.exit(1);
}

const userIds = [...new Set((transfers ?? []).map((row) => row.user_id))];
const { data: profiles } = await db
  .from("profiles")
  .select("id, full_name, email")
  .in("id", userIds);
const profileMap = new Map((profiles ?? []).map((p) => [p.id, p]));

console.log(apply ? "\n=== APPLYING RECONCILIATION ===\n" : "\n=== DRY RUN ===\n");

let updated = 0;

for (const uid of userIds) {
  const { data: wallet } = await db
    .from("investor_portfolios")
    .select("available_balance")
    .eq("user_id", uid)
    .eq("fund_id", DEFAULT_FUND)
    .maybeSingle();

  const legacy = n(wallet?.available_balance);

  const { data: accounts } = await db
    .from("ledger_accounts")
    .select("id, code, account_type")
    .eq("owner_id", uid);

  let ledgerAvailable = 0;
  let reserved = 0;
  let settled = 0;

  for (const acct of accounts ?? []) {
    const bal = await ledgerBalance(acct.id, acct.account_type);
    if (acct.code.includes("AVAILABLE")) ledgerAvailable = bal;
    if (acct.code.includes("RESERVED")) reserved = bal;
    if (acct.code.includes("SETTLED")) settled = bal;
  }

  const hasLedger = ledgerAvailable !== 0 || reserved !== 0 || settled !== 0;
  const target = hasLedger ? roundMoney(Math.max(0, ledgerAvailable)) : legacy;

  if (Math.abs(legacy - target) <= 0.004) continue;

  const profile = profileMap.get(uid);
  console.log(
    JSON.stringify({
      name: profile?.full_name,
      email: profile?.email,
      legacyBefore: legacy,
      ledgerAvailable,
      legacyAfter: target,
    })
  );

  if (apply) {
    const { error: updateError } = await db
      .from("investor_portfolios")
      .update({ available_balance: target })
      .eq("user_id", uid)
      .eq("fund_id", DEFAULT_FUND);
    if (updateError) {
      console.error("Failed:", profile?.email, updateError.message);
    } else {
      updated += 1;
    }
  }
}

console.log(apply ? `\nUpdated ${updated} investor wallet(s).` : "\nRe-run with --apply to write changes.");
